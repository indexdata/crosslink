...
func SendReceiveXml(client *http.Client, url string, buf []byte) ([]byte, error) {
	return requestXml(client, http.MethodPost, url, bytes.NewReader(buf))
}

func GetXml(client *http.Client, url string) ([]byte, error) {
	return requestXml(client, http.MethodGet, url, nil)
}

func requestXml(client *http.Client, method string, url string, reader io.Reader) ([]byte, error) {
	resp, err := clientDo(client, method, url, reader)
	if err != nil {
		return nil, err
	}
