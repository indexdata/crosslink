...
func TestEntryNetworksCases(t *testing.T) {
	...
	cases := []httpTestCase{
		...
		{
			name:        "PATCH entry network",
			method:      http.MethodPatch,
			endpoint:    "/entry-networks/40000000-0000-0000-0000-000000000001",
			status:      http.StatusNoContent,
			bodyFile:    "entry-network.patch.req.json",
			refetchFile: "entry-network.patch.refetch.json",
			addlHeaders: consortiumPermissionHeaders,
		},
		{
			name:        "PATCH entry network rejects network field",
			method:      http.MethodPatch,
			endpoint:    "/entry-networks/40000000-0000-0000-0000-000000000001",
			status:      http.StatusBadRequest,
			body:        `{"network":"ffffffff-ffff-ffff-ffff-ffffffffffff","priority":0}`,
			addlHeaders: consortiumPermissionHeaders,
		},
		{
			name:        "PATCH entry network not found",
			method:      http.MethodPatch,
			endpoint:    "/entry-networks/ffffffff-ffff-ffff-ffff-ffffffffffff",
			status:      http.StatusNotFound,
			body:        `{"priority":0}`,
			addlHeaders: consortiumPermissionHeaders,
		},
		{
			name:        "PATCH entry network without permission",
			method:      http.MethodPatch,
			endpoint:    "/entry-networks/40000000-0000-0000-0000-000000000001",
