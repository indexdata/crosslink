...
func TestIso18626AlmaShimCopyCompleted(t *testing.T) {
	...
	assert.Nil(t, err, "failed to parse xml")
	assert.Equal(t, "URL: http://example.com/item/12345678, sending you the URL", resmsg2.SupplyingAgencyMessage.MessageInfo.Note)
}
...
func TestIso18626AlmaShimLoanLoaned(t *testing.T) {
	...
	assert.Equal(t, LOAN_CONDITION_PRE+string(iso18626.LoanConditionLibraryUseOnly)+", original note\n"+
...
func TestIso18626AlmaShimSupplyingMessageAddLoanConditionWithNote(t *testing.T) {
	...
	assert.Equal(t, LOAN_CONDITION_PRE+"LibraryUseOnly, "+COST_CONDITION_PRE+"20 EUR, staff note", resmsg.SupplyingAgencyMessage.MessageInfo.Note)
}
