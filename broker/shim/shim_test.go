...
func newSAM(sam *iso18626.SupplyingAgencyMessage) *iso18626.ISO18626Message {
	var message = iso18626.NewISO18626Message()
	message.SupplyingAgencyMessage = sam
	return message
}
func newRAM(ram *iso18626.RequestingAgencyMessage) *iso18626.ISO18626Message {
	var message = iso18626.NewISO18626Message()
	message.RequestingAgencyMessage = ram
	return message
}
func newReq(request *iso18626.Request) *iso18626.ISO18626Message {
	var message = iso18626.NewISO18626Message()
...
func TestReShareShimUsesConfiguredOfferedCosts(t *testing.T) {
	...
	_, err := GetShimWithConfig(string(directory.ReShare), ", ", false).ApplyToOutgoingRequest(disabledMessage)
	assert.NoError(t, err)
	assert.Nil(t, disabledMessage.SupplyingAgencyMessage.MessageInfo.OfferedCosts)

	enabledMessage := newMessage()
	_, err = GetShimWithConfig(string(directory.ReShare), ", ", true).ApplyToOutgoingRequest(enabledMessage)
	assert.NoError(t, err)
...
func TestAlmaShimUsesConfiguredNoteFieldSeparator(t *testing.T) {
	shim := GetShimWithConfig(string(directory.Alma), " | ", false).(*Iso18626AlmaShim)
