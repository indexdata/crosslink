...
func (c *Iso18626Client) createAndSendSupplyingAgencyMessage(ctx extctx.ExtendedContext, event events.Event) (events.EventStatus, *events.EventResult) {
	...
	if locSupplier == nil {
		...
		if s, ok := iso18626.StatusMap[lastReceivedStatus.String]; ok {
			// we have a valid status from the supplier
			status = s
			firstMessage = false
		} else if !lastReceivedStatus.Valid {
			// we've not received any status from the supplier (yet)
			if event.EventData.IncomingMessage == nil {
...
func (c *Iso18626Client) createAndSendRequestOrRequestingAgencyMessage(ctx extctx.ExtendedContext, event events.Event) (events.EventStatus, *events.EventResult) {
	...
	selected, supplier, err := c.getSelectedSupplierPeer(ctx, illTrans)
	if err != nil {
		return events.LogErrorAndReturnResult(ctx, FailedToGetSupplier, err)
	}
...
func (c *Iso18626Client) getSelectedSupplierPeer(ctx extctx.ExtendedContext, transaction ill_db.IllTransaction) (*ill_db.LocatedSupplier, *ill_db.Peer, error) {
