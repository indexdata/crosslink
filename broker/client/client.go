...
func (c *Iso18626Client) createAndSendSupplyingAgencyMessage(ctx extctx.ExtendedContext, event events.Event) (events.EventStatus, *events.EventResult) {
	...
	selSupplier, selSupplierPeer, _ := c.getSelectedSupplierAndPeer(ctx, illTrans)
	msgSupplierSymbol := getSymbol(event.EventData.IncomingMessage)
	isNotification := isNotification(event.EventData.IncomingMessage)
	var status iso18626.TypeStatus
	firstMessage := true
	if selSupplier == nil { // no selected supplier
		firstMessage = false
		if isNotification && msgSupplierSymbol != "" {
			// notification on an unfilled transaction
			skipped, skippedPeer, err := c.getSkippedSupplierAndPeer(ctx, illTrans.ID, msgSupplierSymbol)
			if err != nil {
			...
			if requester.BrokerMode == string(extctx.BrokerModeTransparent) {
				selSupplier = skipped
				selSupplierPeer = skippedPeer
				status = iso18626.TypeStatus(illTrans.LastSupplierStatus.String)
			} else {
				return events.LogProblemAndReturnResult(ctx, "invalid-mode",
					fmt.Sprintf("ignored notification for unfilled transaction due to requester mode %s", requester.BrokerMode), nil)
			}
		} else {
			// no suppliers left or found, broker generates unfilled
			status = iso18626.TypeStatusUnfilled
		}
	} else {
		if isNotification && msgSupplierSymbol != "" && msgSupplierSymbol != selSupplier.SupplierSymbol {
			// notification sent by not a selected supplier
			firstMessage = false
			skipped, skippedPeer, err := c.getSkippedSupplierAndPeer(ctx, illTrans.ID, msgSupplierSymbol)
			if err != nil {
				return events.LogErrorAndReturnResult(ctx, FailedToGetSupplier, err)
			...
			if requester.BrokerMode == string(extctx.BrokerModeTransparent) {
				...
				return events.LogProblemAndReturnResult(ctx, "invalid-mode",
					fmt.Sprintf("ignored notification from skipped supplier due to requester mode %s", requester.BrokerMode), nil)
			}
		} else { //there is a selected supplier
			lastReceivedStatus := selSupplier.LastStatus
			if s, ok := iso18626.StatusMap[lastReceivedStatus.String]; ok {
				// we have a valid status from the selected supplier
				status = s
				firstMessage = false
			} else if !lastReceivedStatus.Valid {
				// we've not received any status from the selected supplier (yet)
				if event.EventData.IncomingMessage == nil {
					// supplier did not send this message, the message is generated by the broker
					if requester.BrokerMode == string(extctx.BrokerModeTransparent) || requester.BrokerMode == string(extctx.BrokerModeTranslucent) {
						status = BrokerInfoStatus
					} else {
						resData.Note = fmt.Sprintf("broker does not send %s in mode %s", BrokerInfoStatus, requester.BrokerMode)
						return events.EventStatusSuccess, &resData
					}
				} else {
					// we received a message from the supplier, but it did not contain status nor we registered any status previously
					// we fallback on the default broker status
					status = BrokerInfoStatus
				}
			} else {
				msg := "failed to resolve status for value: " + lastReceivedStatus.String
	...
	message.SupplyingAgencyMessage.Header = c.createMessageHeader(illTrans, selSupplier, false, requester.BrokerMode)
	...
	if status == iso18626.TypeStatusLoaned && appendReturnInfo {
		name, agencyId, address, _ := getPeerInfo(selSupplierPeer, selSupplier.SupplierSymbol)
		populateReturnAddress(message, name, agencyId, address)
	}
	if prependVendor && firstMessage && selSupplierPeer != nil && selSupplierPeer.Vendor != requester.Vendor {
		populateVendor(message.SupplyingAgencyMessage, selSupplierPeer.Vendor)
	}
