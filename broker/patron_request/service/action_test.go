...
func TestHandleInvokeActionReceiveOK(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mockIso18626Handler := new(MockIso18626Handler)
			lmsCreator := new(MockLmsCreator)
			lmsAdapter := &mockLmsAdapter{requesterPickupLocation: tt.configured}
			lmsAdapter.On("AcceptItem", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, tt.expected, "Hold For Pickup").Return(nil).Twice()
			lmsCreator.On("GetAdapter", "ISIL:REC1").Return(lmsAdapter, nil)
			mockEventBus := new(MockEventBus)
...
func TestHandleInvokeLenderActionShipOK(t *testing.T) {
	...
	prAction := CreatePatronRequestActionService(mockPrRepo, new(IllRepoMock), *new(events.EventBus), mockIso18626Handler, lmsCreator, new(EmailSenderMock), nil, nil)
	illRequest := iso18626.Request{}
	mockPrRepo.On("GetPatronRequestById", patronRequestId).Return(pr_db.PatronRequest{ID: patronRequestId, IllRequest: illRequest, State: LenderStateSearching, Side: SideLending, SupplierSymbol: getDbText("ISIL:SUP1"), RequesterSymbol: getDbText("ISIL:REQ1")}, nil)
	mockPrRepo.On("GetItemsByPrId", patronRequestId).Return([]pr_db.Item{
		{
			ID:        "item1",
			PrID:      patronRequestId,
...
func (r *MockPrRepo) SaveItem(ctx common.ExtendedContext, params pr_db.SaveItemParams) (pr_db.Item, error) {
	...
	item := pr_db.Item{ID: params.ID, PrID: params.PrID, Barcode: params.Barcode, CallNumber: params.CallNumber, Title: params.Title, ItemID: params.ItemID, LmsRequestID: params.LmsRequestID, LmsItemID: params.LmsItemID, CreatedAt: params.CreatedAt, LmsStatus: pr_db.LmsStatusUnknown}
	r.savedItems = append(r.savedItems, item)
	return item, nil
}

func (r *MockPrRepo) SetItemLmsStatus(ctx common.ExtendedContext, params pr_db.SetItemLmsStatusParams) error {
	for i := range r.savedItems {
		if r.savedItems[i].ID == params.ID {
			r.savedItems[i].LmsStatus = params.LmsStatus
			r.savedItems[i].LmsDueDate = params.LmsDueDate
		}
	}
	return nil
}
...
func (l *MockLmsAdapterFail) AcceptItem(
	...
	return false, errors.New("AcceptItem failed")
}

func (l *MockLmsAdapterFail) DeleteItem(itemId string) (bool, error) {
	return false, errors.New("DeleteItem failed")
}
...
func (l *MockLmsAdapterFail) CheckInItem(itemId string) (bool, error) {
	return false, errors.New("CheckInItem failed")
}
...
) (*lms.CheckedOutItem, error) {
	return nil, errors.New("CheckOutItem failed")
}
...
func (m *mockLmsAdapter) CheckOutItem(
	...
	if result, ok := args.Get(0).(*lms.CheckedOutItem); ok {
		return result, args.Error(1)
	}
	return &lms.CheckedOutItem{Title: args.String(0)}, nil
}
...
func (m *mockLmsAdapter) AcceptItem(
	...
	return args.Error(0) == nil, args.Error(0)
}

func (m *mockLmsAdapter) DeleteItem(itemId string) (bool, error) {
	err := m.Called(itemId).Error(0)
	return err == nil, err
}
...
func (i *IllRepoMock) GetCachedPeersBySymbols(ctx common.ExtendedContext, symbols []string, directoryAdapter adapter.DirectoryLookupAdapter) ([]ill_db.Peer, string, error) {
	...
	config := &dirapi.IllConfig{}
	config.DefaultLoanPeriod.Set(30)
	return []ill_db.Peer{{Vendor: "other", CustomData: dirapi.Entry{IllConfig: config}}}, "", nil
}
...
func TestPullslipPrintedAndShippingGate(t *testing.T) {
	for _, state := range []pr_db.PatronRequestState{LenderStateWillSupply, LenderStateSearching, LenderStateShipped} {
		t.Run(string(state), func(t *testing.T) {
			repo := new(MockPrRepo)
			pr := pr_db.PatronRequest{ID: patronRequestId, Side: SideLending, State: state}
			repo.On("GetPatronRequestById", patronRequestId).Return(pr, nil)
			svc := CreatePatronRequestActionService(repo, nil, new(MockEventBus), nil, nil, nil, nil, nil)
			action := LenderActionPullslipPrinted
			status, _ := svc.handleInvokeAction(appCtx, events.Event{PatronRequestID: pr.ID, EventData: events.EventData{CommonEventData: events.CommonEventData{Action: &action}}})
			if state == LenderStateShipped {
				assert.NotEqual(t, events.EventStatusSuccess, status)
			} else {
				assert.Equal(t, events.EventStatusSuccess, status)
				assert.Equal(t, LenderStateSearching, repo.savedPr.State)
