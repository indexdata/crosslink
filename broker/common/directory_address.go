...
// DirectoryShippingAddress converts the first Shipping address with non-nil AddressComponents to ISO18626.
// An empty component list returns an empty address; later addresses are ignored.
func DirectoryShippingAddress(entry dirapi.Entry) iso18626.PhysicalAddress {
	...
	if entry.Addresses != nil {
		for _, addr := range *entry.Addresses {
			if addr.Type != "Shipping" || addr.AddressComponents == nil {
				continue
			}
