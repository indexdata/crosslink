...
func TestApplyRequesterShimError(t *testing.T) {
	...
	err := applyRequesterShim(appCtx, mockRepo, "1", iso18626.NewIso18626MessageNS(), &eventData, nil)
...
func TestApplyRequesterShimAlma(t *testing.T) {
	...
	assert.NotNil(t, eventData.CustomData[ORIGINAL_INCOMING_MESSAGE])
	assert.Equal(t, "BROKER", eventData.CustomData[ORIGINAL_INCOMING_MESSAGE].(*iso18626.Iso18626MessageNS).RequestingAgencyMessage.Header.SupplyingAgencyId.AgencyIdValue)
}
